            ...
            parameters: {
                ...
                properties: {
                    ...
                    page: {
                        type: "string",
                        description:
                            "Page number for the quote, as a string. Use a numeric string for a single page (e.g. \"3\"). Use \"N-M\" (e.g. \"41-42\") if the quote spans two pages, and insert [[PAGE_BREAK]] in the quote at the page break. Refers to the [Page N] markers in the text — not page numbers printed inside the document.",
                    },
                    quote: {
                        type: "string",
                        description:
...
function normalizeCitation(raw: unknown): ParsedCitation | null {
    ...
    // ref/marker may arrive as int OR string-of-digits depending on the LoRA's
    // tool-arg serialisation. Coerce string-of-digits to int.
    let ref: number;
    if (typeof c.ref === "number" && Number.isFinite(c.ref)) {
        ref = c.ref;
    } else if (typeof c.ref === "string" && /^\d+$/.test(c.ref.trim())) {
        ref = parseInt(c.ref.trim(), 10);
    } else {
        return null;
    }
    ...
    return { ref, doc_id: c.doc_id, page, quote: c.quote };
}
