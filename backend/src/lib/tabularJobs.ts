...
The "summary" field must contain only the extracted value with inline citations — no explanation or reasoning. Every distinct fact (every name, date, number, party, clause, defined term, or substantive statement) MUST be followed immediately by its own [[page:N||quote:exact quoted text]] marker, where N is the page number and the quote is a short verbatim excerpt (≤ 25 words). Lists and bulleted summaries are not exempt: if the summary lists three parties, emit three citation markers — one per party. If the answer is "Not Found", no citations are needed. Quotes must be narrowly scoped to the specific claim — extract only the exact supporting words, not the surrounding sentence. Do not reuse one long quote across multiple statements. Do NOT use prose page references like "(Page 5)" or "see Section 3" inside the summary — always use the [[page:N||quote:...]] format so the UI can render clickable chips. Example for a Parties column: "ACME CORP [[page:1||quote:ACME CORP]] and BETA INC [[page:1||quote:BETA INC]]". All reasoning and explanation belongs in "reasoning" only, which may also contain [[page:N||quote:...]] citations.

The "keywords" field is an array of 3–5 short, distinct phrases (each ≤4 words) a reviewer could type into a doc-search box to verify this answer in the source document. Prefer terms that appear verbatim or near-verbatim in the document — section names, defined terms, key clause-anchor phrases, party names. Skip generic words ("provision", "clause", "agreement") and skip anything that just restates the column name. If you cannot suggest useful search terms, return an empty array.`;
...
Rules:
- "summary": the extracted value, with inline citations [[page:N||quote:verbatim excerpt ≤25 words]] after EVERY distinct fact. This is non-negotiable — every name, date, number, party, clause, defined term, or substantive statement MUST be followed by its own [[page:N||quote:...]] marker. Lists and bulleted summaries are not exempt: if the summary lists three parties, emit three citation markers (one per party). If the answer is "Not Found", no citations are needed. Quotes must be narrowly scoped to the specific claim — extract only the exact supporting words, not the surrounding sentence. Do not reuse one long quote across multiple statements; give each claim its own short, precise quote. Do NOT use prose page references like "(Page 5)" or "see Section 3" inside the summary — always use the [[page:N||quote:...]] format so the UI can render clickable chips. Example for a Parties column: "ACME CORP [[page:1||quote:ACME CORP]] and BETA INC [[page:1||quote:BETA INC]]".
- "flag": green = standard/favorable, yellow = needs attention, red = problematic/unfavorable, grey = neutral/not found
- "reasoning": brief explanation of the extraction. May also include [[page:N||quote:...]] citations and is encouraged to do so when explaining where evidence lives.
- "keywords": 3–5 short, distinct phrases (each ≤4 words) a reviewer could type into a doc-search box to verify this answer. Prefer terms that appear verbatim or near-verbatim in the document — section names, defined terms, key clause-anchor phrases, party names. Skip generic words ("provision", "clause", "agreement") and skip anything that just restates the column name. If you cannot suggest useful search terms, return an empty array.
