...
function DocVersionHistory({
    ...
    if (loading && versions.length === 0) {
        return (
            <div className="flex items-center h-9 border-b border-border/50 text-xs text-muted-foreground bg-muted/60">
    ...
    if (versions.length === 0) {
        return (
            <div className="flex items-center h-9 border-b border-border/50 text-xs text-muted-foreground/70 bg-muted/60">
                <div className={`sticky left-0 z-[60] ${CHECK_W} bg-muted/60 self-stretch`} />
                <div className={`sticky left-8 z-[60] ${NAME_COL_W} bg-muted/60 p-2`}>
                    <div>
                        No version history.
            ...
            {ordered.map((v) => {
                ...
                return (
                    <div
                        ...
                        className="group flex items-center h-9 pr-8 border-b border-border/50 bg-muted/60 text-xs text-muted-foreground cursor-pointer hover:bg-muted/80 transition-colors"
                    >
                        <div className={`sticky left-0 z-[60] ${CHECK_W} bg-muted/60 group-hover:bg-muted/80 self-stretch`} />
                        <div className={`sticky left-8 z-[60] ${NAME_COL_W} bg-muted/60 group-hover:bg-muted/80 p-2`}>
                        <div className="flex items-center gap-2">
                            ...
                            {isEditing ? (
                                <input
                                    ...
                                    className="min-w-0 flex-1 max-w-[240px] border-b border-border bg-transparent text-xs text-foreground outline-none focus:border-ring"
                                />
                            ) : (
                            ...
                            {!isEditing && onRenameVersion && (
                                <button
                                    ...
                                    className="shrink-0 rounded p-0.5 text-muted-foreground/70 opacity-0 group-hover:opacity-100 hover:text-foreground hover:bg-secondary transition"
                                >
    ...
    function renderFolderInput(parentId: string | null) {
        ...
        return (
            <div
                ...
                className="group flex items-center h-10 pr-8 border-b border-border/50"
                key={`new-folder-${parentId ?? "root"}`}
            >
                <div className={`sticky left-0 z-[60] ${CHECK_W} bg-card self-stretch`} />
                <div className={`sticky left-8 z-[60] ${NAME_COL_W} bg-card p-2`}>
                ...
                {childDocs.map((doc) => {
                    ...
                    return (
                        <div key={`doc-${doc.id}`}>
                            ...
                            className="group flex items-center h-10 pr-8 border-b border-border/50 hover:bg-muted cursor-pointer transition-colors"
                            >
                                {(() => {
                                ...
                                <div className="ml-auto w-20 shrink-0 text-xs text-muted-foreground uppercase truncate">
                                    {doc.file_type ?? <span className="text-muted-foreground/50">—</span>}
                                </div>
                                <div className="w-24 shrink-0 text-sm text-muted-foreground truncate">
                                    {doc.size_bytes != null ? formatBytes(doc.size_bytes) : <span className="text-muted-foreground/50">—</span>}
                                </div>
                                <div
                                ...
                                >
                                    {hasVersions ? (
                                        ...
                                        <span className="text-muted-foreground/50 pl-1">—</span>
                                    )}
                                </div>
                                <div className="w-32 shrink-0 text-sm text-muted-foreground truncate">
                                    {doc.created_at ? formatDate(doc.created_at) : <span className="text-muted-foreground/50">—</span>}
                                </div>
                                <div className="w-32 shrink-0 text-sm text-muted-foreground truncate">
                                    {doc.updated_at ? formatDate(doc.updated_at) : <span className="text-muted-foreground/50">—</span>}
                                </div>
                                <div className="w-8 shrink-0 flex justify-end">
                                    {!isProcessing && (
                                        <RowActions
                    ...
                    return (
                        <div key={`folder-${folder.id}`}>
                            <div
                                ...
                                className={`group flex items-center h-10 pr-8 border-b border-border/50 hover:bg-muted cursor-pointer transition-colors select-none ${dragOverFolderId === folder.id ? "bg-blue-50 ring-1 ring-inset ring-blue-200" : ""}`}
                            >
                                <div className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${dragOverFolderId === folder.id ? "bg-blue-50" : "bg-card"} group-hover:bg-muted self-stretch`}>
                                    {isExpanded
                                ...
                                <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${dragOverFolderId === folder.id ? "bg-blue-50" : "bg-card"} group-hover:bg-muted`}>
                                <div className="flex items-center gap-1.5">
                                    {isExpanded
                                ...
                                <div className="ml-auto w-20 shrink-0 text-xs text-muted-foreground/50">—</div>
                                <div className="w-24 shrink-0 text-sm text-muted-foreground/50">—</div>
                                <div className="w-20 shrink-0 text-sm text-muted-foreground/50">—</div>
                                <div className="w-32 shrink-0 text-sm text-muted-foreground/50">—</div>
                                <div className="w-32 shrink-0 text-sm text-muted-foreground/50">—</div>
                                <div
    ...
    if (loading) {
        return (
            <div className="flex-1 overflow-y-auto bg-card">
                ...
                {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="flex items-center h-10 pr-8 border-b border-border/50">
                        <div className="w-8 shrink-0" />
    ...
    if (!project) {
        return (
            <div className="flex h-full items-center justify-center">
                <p className="text-muted-foreground/70">Project not found</p>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative group">
                        <button
                            ...
                            className={`flex h-8 items-center justify-center gap-1.5 px-3 text-sm transition-colors ${
                                !creatingChat ? "text-muted-foreground hover:text-foreground cursor-pointer" : "text-muted-foreground/50 cursor-default"
                            }`}
                        >
                    ...
                    <div className="relative group">
                        <button
                            ...
                            className={`flex h-8 items-center justify-center gap-1.5 px-3 text-sm transition-colors ${
                                docs.length > 0 ? "text-muted-foreground hover:text-foreground cursor-pointer" : "text-muted-foreground/50 cursor-default"
                            }`}
                        >
                        ...
                        {docs.length === 0 && (
                            <div className="pointer-events-none absolute right-0 top-full mt-1.5 z-10 hidden group-hover:flex items-center whitespace-nowrap rounded-lg bg-foreground px-2.5 py-1.5 text-xs text-primary-foreground shadow-lg">
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        {docs.length === 0 && folders.length === 0 ? (
                            ...
                            >
                                ...
                                <p className="text-sm text-muted-foreground/70">Drop PDF or DOCX files here</p>
                            </div>
                        ) : (
                            <div
                                ...
                                {q ? (
                                    ...
                                    setViewingDoc(doc);
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {isProcessing ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground/70 shrink-0" /> : isError ? <AlertCircle className="h-4 w-4 text-red-500 shrink-0" /> : <DocIcon fileType={doc.file_type} />}
                                                        <span className="text-sm text-foreground truncate">{doc.filename}</span>
                                                    </div>
                                                    </div>
                                                    <div className="ml-auto w-20 shrink-0 text-xs text-muted-foreground uppercase truncate">{doc.file_type ?? <span className="text-muted-foreground/50">—</span>}</div>
                                                    <div className="w-24 shrink-0 text-sm text-muted-foreground truncate">{doc.size_bytes != null ? formatBytes(doc.size_bytes) : <span className="text-muted-foreground/50">—</span>}</div>
                                                    <div
                                                        className="w-20 shrink-0 text-sm text-muted-foreground flex items-center gap-1"
                                                        onClick={(e) => e.stopPropagation()}
                                                    >
                    ...
                    <>
                        ...
                        {chats.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                ...
                                <p className="mt-1 text-xs text-muted-foreground/70 max-w-xs">Ask questions and get answers grounded in the documents in this project.</p>
                                <button onClick={() => handleNewChat()} className="mt-4 inline-flex items-center gap-1 rounded-full bg-foreground px-3 py-1 text-xs font-medium text-primary-foreground hover:bg-foreground transition-colors shadow-md">
                                    + Create New
                                </button>
                            </div>
                        ) : (
                    ...
                    <>
                        ...
                        {projectReviews.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                ...
                                <p className="mt-1 text-xs text-muted-foreground/70 max-w-xs">Extract data from project documents into tables using AI.</p>
                                <button onClick={handleNewReview} disabled={creatingReview || docs.length === 0} className="mt-4 inline-flex items-center gap-1 rounded-full bg-foreground px-3 py-1 text-xs font-medium text-primary-foreground hover:bg-foreground transition-colors shadow-md disabled:opacity-40">
                                    + Create New
                                </button>
                            </div>
                        ) : (
                            <div>
                                {filteredReviews.map((review) => (
                                    ...
                                    >
                                        ...
                                        <div className="w-32 shrink-0 text-sm text-muted-foreground truncate">{review.created_at ? formatDate(review.created_at) : <span className="text-muted-foreground/50">—</span>}</div>
                                        <div className="w-8 shrink-0 flex justify-end" onClick={(e) => e.stopPropagation()}>
                                            <RowActions
                                                onRename={() => {
